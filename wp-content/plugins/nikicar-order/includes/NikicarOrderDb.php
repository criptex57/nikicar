...
class NikicarOrderDb {
	 ...
	 * @return float|int
	 */
	static public function getTotalPrice($userOrder){
		$orderProducts = json_decode( $userOrder, true );
		$totalPrice = 0;
	...
	static public function changeOrder($orderId, $status){
		...
		if($order && $order['status'] != $status){
			...
			switch($status){
				case self::ORDER_STATUS_APPROVE:
					NikicarBotClass::approveOrderMessage($orderId);
					break;
				case self::ORDER_STATUS_PAID:
					NikicarBotClass::paidOrderMessage($orderId);
					break;
				case self::ORDER_STATUS_CLOSE:
					NikicarBotClass::closeOrderMessage($orderId);
					break;
			}
		}
	}

	static public function addTelegramMessageId($orderId, $messageId){
		$t = self::getTableName();
		self::getWpdb()->get_results("UPDATE `{$t}` SET `tgMessageId` = {$messageId} WHERE `{$t}`.`id` = {$orderId};", ARRAY_A);
	}
	...
	static public function checkTable(){
		if(self::getWpdb()->get_var("show tables like '".self::getTableName()."'") != self::getTableName()) {
			...
			$sql = "CREATE TABLE {$tn} (
			  ...
			  tgMessageId mediumint(9) NOT NULL,
